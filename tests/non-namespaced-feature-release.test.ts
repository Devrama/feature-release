...
describe('initFeatureRelease() with non-namespaced cases', () => {
  ...
  beforeAll(async () => {
    featureFlagInstance = await initFeatureRelease(nonNamespacedConfig);
  });
  ...
  it('cannot reinitialize', async () => {
    expect(() => initFeatureRelease(nonNamespacedConfig)).toThrowError(
